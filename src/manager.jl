...
function Distributed.launch_on_machine(manager::MulticlusterSSHManager, machine::AbstractString, manager_program, worker_programs, params::Dict, launched::Array, launch_ntfy::Condition)
    ...
    if shell === :posix
        ...
        cmds = "exec mpiexec -hostfile $(Base.shell_escape_posixly(hostfile)) --map-by node -env UCX_WARN_UNUSED_ENV_VARS n -np 1 $(Base.shell_escape_posixly(exename)) $(Base.shell_escape_posixly(exeflags)) $(Base.shell_escape_posixly(other_processes))"
